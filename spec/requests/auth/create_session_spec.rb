...
RSpec.describe "auth/create_session" do
  before { post "/auth/create_user", params: Protocol::CreateUserRequest.new(device_id: device_id, name: SecureRandom.alphanumeric), as: :protobuf }
  subject { post "/auth/create_session", params: request, as: :protobuf }
  let(:device_id) { SecureRandom.uuid }

  context 'when valid request' do
    let(:request) { Protocol::CreateSessionRequest.new(device_id: device_id) }

    it 'returns ok response' do
      subject

      expect(response).to have_http_status 200
      resp = Protocol::CreateSessionResponse.decode(response.body)
  ...
  context 'when invalid request' do
    context "when device_id is blank" do
      let(:request) { Protocol::CreateSessionRequest.new(device_id: SecureRandom.uuid) }
